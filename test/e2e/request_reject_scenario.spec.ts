...
test("send request and reject by judger", async () => {
    ...
    const res = await helper.addRequest({
        projectId: projectId,
        email: user.email,
        roles: roles,
        reason: reason,
    ...
    const judger = await utils.createUser({
        role: "judger",
        expiredAt: expiredAt,
    });
    ...
    await judgeRes.page.locator(".status", { hasText: "rejected" }).waitFor();
});
