...
test("after filter role, request form role is filtered", async () => {
    ...
    await page
        .getByPlaceholder("Enter a keyword between 3 and")
        .fill("spanner");
    await page.getByRole("button", { name: "Add Keyword" }).click();
    // Check if 'spanner' is added to the <td>
    expect(await (await page.waitForSelector("td")).innerText()).toContain(
        "spanner",
    );
            ...
            response
                ...
                .then((json) =>
                    json.iamRoles
                        .map(
                            (iamRole: string) =>
                                iamRole.includes("spanner") ||
                                iamRole.includes("bigquery"),
                        )
                        .every((v: boolean) => v),
                ),
    );
