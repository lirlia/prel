...
export const addRequest = async (
    req: prelRequestInput,
): Promise<replRequestOutput> => {
    const page = await req.ctx.newPage();
    const iamResponsePromise = page.waitForResponse(
        (response) =>
    ...
    Promise.all([
        ...
        expect(
            await page.textContent("#select2-project_id-container"),
        ).toContain(req.projectId),
        // open role dropdown
...
export const judgeRequestInSpecificPage = async (
    req: prelJudgeRequestInput,
): Promise<prelJudgeRequestOutput> => {
    const page = await req.ctx.newPage();
