				...
				It("should update expected request", func() {
					...
					helper.GoogleCloudResourceManagerSrv.EXPECT().
						...
						DoAndReturn(func(projectID string, req *cloudresourcemanager.SetIamPolicyRequest) google_cloud.IamPolicySetter {
							...
							Expect(req.Policy.Bindings[0].Condition).NotTo(BeNil())
							Expect(req.Policy.Bindings[0].Condition.Title).To(Equal("generated_by_prel_expires_after_2021_01_01_00_10_UTC"))
							Expect(req.Policy.Bindings[0].Condition.Description).To(Equal("Expiring at 2021/01/01 00:10:00 UTC generated by prel"))
							Expect(req.Policy.Bindings[0].Condition.Expression).To(Equal("request.time < timestamp(\"2021-01-01T00:10:00Z\")"))
